...
async function clearAndStartNewAnalysis() {
    try {
        ...
        if (data.success) {
            ...
            // Use URL provided by template (set in recommendations.html) or fallback to route path
            window.location.href = window.EYE_ANALYSIS_URL || '/eye-analysis';
        } else {
            // Show error
...
function exportRecommendations() {
    // Request the server to generate and return a PDF report for the logged-in user.
    // The server endpoint '/download-report' returns an attachment PDF when authenticated.
    try {
        // Option A: navigate to the download endpoint which will trigger a file download
        window.location.href = '/download-report';
    } catch (err) {
        console.error('Failed to download PDF report:', err);
        alert('Could not download report. Please try again or contact support.');
    }
}
